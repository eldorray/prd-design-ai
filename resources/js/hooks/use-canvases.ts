...
export const ALL_KINDS: DesignKind[] = [
    'landing-page',
    'dashboard',
    'mobile-app',
];
...
export function useCanvases(current: Design | null) {
    ...
    const [canvases, setCanvases] = useState<Record<DesignKind, CanvasState>>(
        init.canvases,
    );
    const [selectedKinds, setSelectedKinds] = useState<DesignKind[]>(
        init.selectedKinds,
    );
    const [activeKind, setActiveKind] = useState<DesignKind>(
        init.selectedKinds[0],
    );
    ...
    const toggleKind = useCallback(
        (kind: DesignKind) => {
            ...
            if (!isOn) {
                setCanvases((c) =>
                    c[kind] ? c : { ...c, [kind]: emptyCanvas(kind) },
                );
            }

            setActiveKind((current) =>
                ordered.includes(current) ? current : ordered[0],
            );
        },
        [selectedKinds],
    );
